...
def main():
    ...
    parser.add_argument('--cache', type=str, required=True,
                       help='Path to preprocessed data cache (.npz file)')
    ...
    parser.add_argument('--n_folds', type=int, default=10,
                       help='Number of folds to evaluate (default: 10)')
    ...
    for fold_idx in range(args.n_folds):
        ...
        Xr_test = Xr[test_idx]
        y_test = y[test_idx]
        ilddt_test = ilddt[test_idx]
        
        # Create test dataset
        test_dataset = DdgDataset(Xf_test, Xr_test, y_test, ilddt_test)
        test_loader = DataLoader(test_dataset, batch_size=args.batch_size, shuffle=False)
        
        # Initialize model with hardcoded architecture
        ...
        # Evaluate
        pred_ddg, true_ddg, pred_ilddt, true_ilddt = evaluate_model(
