...
def find_ost_json(pdb_path: str, ost_json_dir: str) -> Optional[str]:
    """
    Find the OST JSON file corresponding to a PDB file.
    
    Parameters:
        pdb_path: Path to the PDB file (e.g., "data/optimized/123_mutant.pdb")
        ost_json_dir: Directory containing OST JSON files
    
    Returns:
        Path to JSON file if found, None otherwise
    """
    pdb_basename = Path(pdb_path).stem  # e.g., "123_mutant"
    json_path = os.path.join(ost_json_dir, f"{pdb_basename}.json")
...
def load_local_lddt_data(json_path: str) -> Tuple[Dict[tuple, float], float]:
    """
    Load local lDDT data and global ilddt from OST JSON file.
    
    Returns:
        Tuple of:
        - Dictionary mapping (chain, resnum) -> local_lddt_score
        - Global ilddt value (or np.nan if not found)
    """
    ...
    # OST JSON structure has 'local_lddt' as a list of objects
    if 'local_lddt' in data:
        for residue_data in data['local_lddt']:
