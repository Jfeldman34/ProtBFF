...
def main():
    ...
    ap.add_argument('--ca_base', required=True, help='pretrained cross-attention (ProtBFF) base checkpoint')
    ap.add_argument('--simple_base', required=True, help='pretrained Simple (no-scores) base checkpoint')
    ap.add_argument('--cache', required=True, help='scaled score cache for the ProtBFF model (5*embed_dim)')
    ap.add_argument('--merged', required=True, help='merged dir (unscaled per-residue embeddings) for the Simple model')
    ap.add_argument('--out', required=True, help='output dir')
    ap.add_argument('--embed_dim', type=int, default=768)
    args = ap.parse_args()
    CROSS_ATTN_BASE_MODEL = args.ca_base
    SIMPLE_BASE_MODEL = args.simple_base
    CACHE_PATH = args.cache
    COMBINED_NPZ_DIR = args.merged
    OUTPUT_DIR = args.out
    EMBED_DIM = args.embed_dim
    
    # Training set sizes to test: 0% to 80% at 10% intervals
    ...
    print("\nConfiguration:")
    print(f"  Cross-Attn model: {CROSS_ATTN_BASE_MODEL}")
    print(f"  Simple model:     {SIMPLE_BASE_MODEL}")
    print(f"  Cache:            {CACHE_PATH}")
    print(f"  NPZ dir:          {COMBINED_NPZ_DIR}")
    print(f"  Output dir:       {OUTPUT_DIR}")
    print(f"  Train ratios:     {[f'{r*100:.0f}%' for r in TRAIN_RATIOS]}")
    ...
    Xf_ca, Xr_ca, y_ca, ids_ca = load_cross_attention_data(CACHE_PATH)
    Xf_simple, Xr_simple, y_simple, ids_simple = load_simple_data(COMBINED_NPZ_DIR)
    ...
    ca_base_model = FullModelCrossAttention(input_dim=5 * EMBED_DIM).to(device)
    ca_checkpoint = torch.load(CROSS_ATTN_BASE_MODEL, map_location=device, weights_only=False)
    ...
    simple_base_state = torch.load(SIMPLE_BASE_MODEL, map_location=device)
    ...
    for train_ratio in TRAIN_RATIOS:
        ...
        if train_ratio == 0.0:
            ...
            # Cross-Attention
            ca_model = FullModelCrossAttention(input_dim=5 * EMBED_DIM).to(device)
            ...
            train_loader_ca = DataLoader(train_dataset_ca, batch_size=BATCH_SIZE_CA, shuffle=True)
            ...
            train_loader_simple = DataLoader(train_dataset_simple, batch_size=BATCH_SIZE_SIMPLE, shuffle=True)
        ...
        if train_ratio > 0.0:
            ca_model_path = os.path.join(OUTPUT_DIR, f'cross_attn_model_{int(pct)}pct.pth')
            torch.save(ca_model.state_dict(), ca_model_path)
            
            simple_model_path = os.path.join(OUTPUT_DIR, f'simple_model_{int(pct)}pct.pth')
    ...
    plot_comparison_metrics(cross_attn_results, simple_results, OUTPUT_DIR)
    plot_scatter_comparison(cross_attn_results, simple_results, OUTPUT_DIR)
    ...
    for pct in sorted(cross_attn_results.keys()):
        ...
        pred_path = os.path.join(OUTPUT_DIR, f'predictions_{int(pct)}pct.csv')
